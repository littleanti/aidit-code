...
- **점검 결과(이상 없음)**: 작업 트리 clean · 세션 커밋 10개 · `진행중` 0건 ·
  제품/테스트 코드에 `TODO`/`FIXME`/`test.skip`/`.only` **0건** ·
  백엔드 36파일/152건 · 프론트 9파일/127건 · 양쪽 typecheck · 프론트 빌드 · keygate 전부 통과.
- **발견한 문서 결함 2건(GR-1 위반 — 코드는 바뀌었는데 상위 문서가 안 따라옴)**:
  1. **PRD 가 락 입도를 여전히 "샌드박스 단위"로 서술**한다(§Δv2 표·NFR-8·A-5·리스크 표 등).
     XC-SCOPE 로 **충돌 단위**(FILE_WRITE/READ=경로별, SHELL/PACKAGE/FILE_DELETE=샌드박스 배타)로
     바뀌었고 TRD §6.4 는 갱신했으나 PRD 는 누락했다. 더 문제인 것은 PRD 가
     *"필요 시 같은 파일 턴만 직렬 폴백"* 을 **미래 옵션**으로 적어둔 점이다 — 이미 구현·검증됐다.
  2. **PLAN 릴리스 체크리스트의 M8 항목이 미체크(`[ ]`)** 로 남아 있다(다른 15개는 `[x]`).
     README 는 "M1~M8 전부 완료"라고 적어 **두 문서가 모순**이다. 이제 FE-MULTI 단언형 E2E 가
     브라우저에서 M8 종료 기준을 실증하므로 근거와 함께 체크한다.
     같은 줄의 테스트 수 서술(`52 vitest`)도 실제(백 152 + 프론트 127)와 어긋나 갱신한다.
- **정정 내용**:
  - `PRD.md` — §Δv2 비교표·알려진 한계·NFR-8·A-5·리스크 표 5곳을 **충돌 단위** 락으로 정정하고,
    "같은 파일 턴만 직렬 폴백"을 *미래 옵션*에서 **구현 완료**로 바꿨다. 성능 이득 0 사실과
    `LOCK_SCOPE` 롤백 스위치도 함께 명시했다. `last-wins` 는 이 변경과 무관하게 남는 한계임을 구분해 적었다.
  - `PLAN.md` — M8 체크리스트 항목을 **완료 근거(실측 3종)와 함께 체크**, 테스트 수를 실제값으로 갱신,
    **후속 과제 절 신설**(논문 실험 잔여·운영 담당·PATENT/PAPER 정합 미처리).
  - `README.md` — 문서 표에 **EXPERIMENTS·BUSINESS_VALUE·PATENT·PAPER 누락분 추가**(심사자가 증거
    문서를 찾을 수 있도록) + "핵심 주장의 증거는 EXPERIMENTS.md" 안내.
- **수치 교차검증(문서가 거짓말하지 않는지 원자료와 대조)** — 전부 일치:
  - E2: 기울기 `{C-FIFO:1, C-PAR:0, C-REJECT:1.01}`, 180런 실패 0, L=15s p50 14930/235 → **63.5×** ✓
  - E1: `LOCK=on` 158스냅샷·mixed 0·0.00%·intact×3 / `LOCK=off` 50·mixed 37·**74.00%**·mixed×3 ✓
  - Docker PoC: PASS 7 · FAIL 0 · MEASURED 1 · 격차 3건(network/host-fs/memory) ✓
- **런타임 재검증**: 단언형 브라우저 E2E **9/9 통과**, 참조 파일 14개 전부 존재.
- 변경 파일: `docs/PRD.md`, `docs/PLAN.md`, `README.md`, `docs/IMPLEMENTATION_NOTES.md`.

### 2026-07-28 · [test] · 완료 · E1 직렬 실행기 Ablation — "락을 빼면 진짜로 깨지는가"
- **요청(사용자)**: 개선안 1번. `EXPERIMENTS.md` E1 을 설계에서 실측으로 격상.
- **동기**: v2 의 안전 주장("부수효과 직렬화 → 동시 파일 쓰기 진입 0")을 받치는 근거가 없다.
  락이 **있을 때 0** 이라는 것만으로는 불충분하다 — 락을 **빼면 실제로 깨진다**는 대조가 있어야
  "락이 장식이 아니다"가 증명된다.
- **측정 층 선택(설계 §E1 준수)**: 에이전트/LLM 스택을 통과시키지 않고 `executeTool` + 락 프리미티브를
  **직접** 구동한다. E1 은 모델 행동이 아니라 **I/O 경합**을 재는 실험이므로 결정적 백엔드가 옳다
  (설계 문서가 명시한 방침). 서버·DB·SSE 노이즈가 제거돼 감도가 최대가 된다.
- **워크로드 W1(파일 원자성)**: 두 "턴"이 **같은 파일**에 K회 전체 쓰기를 동시에 수행한다.
  페이로드 구조 = `HEADER(writerId,seq)` + 본문(패턴 반복) + `FOOTER(본문 SHA-256)`.
  대용량(기본 1 MiB)으로 두어 단일 `write()` 원자성에 가려지지 않게 한다.
  - **검증기 분류**: ① 무결(한 writer 의 헤더·본문·푸터 체크섬 일치) ② 혼합(두 writer 마커 공존
    = torn write) ③ 깨짐(체크섬 불일치/구조 파손). 지표 = ②+③ 발생률.
  - **감시 방식**: 최종 파일만 보면 중간 torn 상태를 놓치므로, 쓰기 진행 중 **파일을 반복 읽어
    스냅샷마다 분류**한다(설계의 "각 쓰기 직후·최종 파일을 파싱"을 폴링으로 구현).
- **조작 변인**: `LOCK=on|off`. off 는 락을 우회(통과)시킨다 — **벤치 스크립트 내부에서만** 우회하며
  제품 코드에 우회 경로를 만들지 않는다(`src/` 무수정).
- **감도 주의(설계에 명시된 위험)**: off 에서도 0 이 나오면 OS 단일 `write()` 원자성에 가려진 것이므로
  페이로드를 4 MiB 로 올려 재시도한다. 그래도 0 이면 **그 사실 자체를 정직히 보고**하고
  "플랫폼 원자성 한계 내에서는 안전"이라고 기록한다(억지로 실패를 만들지 않는다).
- **실측 결과: CONFIRMED** (writer 4명 × 각 12회 × 페이로드 4 MiB, 같은 파일, 반복 3회 합산):

  | 조건 | 분류 스냅샷 | mixed(torn) | broken | **위반률** | 최종 파일 |
  |---|---|---|---|---|---|
  | `LOCK=on`(현행) | 158 | **0** | 0 | **0.00%** | intact ×3 |
  | `LOCK=off`(ablation) | 50 | **37** | 0 | **74.00%** | **mixed ×3** |

  - 락을 빼면 **3회 모두 최종 파일 자체가 오염**됐다(두 writer 마커 공존). 중간 상태만 잠깐
    깨지는 게 아니라 **결과물이 망가진다** — "락이 장식이 아니다"가 실증됐다.
  - `LOCK=on` 의 158개가 전부 `partial` 인 것은 정상(writeFile 은 truncate 후 기록). **mixed=0**,
    최종 파일은 3회 모두 체크섬까지 일치.
- **감도에 대한 정직한 기록**: 첫 시도(512 KiB · writer 2명)에서는 `LOCK=off` 에서도 위반이 **0** 이었다 —
  설계가 미리 경고한 "OS 단일 `write()` 원자성에 가려짐" 그대로다. 지침대로 **4 MiB · writer 4명**으로
  올려 위 결과를 얻었다. 즉 이 결함은 **충분히 큰 쓰기에서만** 드러난다. 그 사실이 곧
  "락이 왜 필요한가"의 정확한 답이다 — **크기에 의존하지 않는 보장**을 준다.
- **미실시**: W1b(디렉토리 레이스), W2(convo 짝 정합 — 워커 커밋 경로에 실험 게이트가 필요).
- 변경 파일: `backend/bench/e1-ablation.mjs`(신규), `backend/bench/out/e1-ablation.json`,
  `backend/package.json`, `docs/EXPERIMENTS.md`(E1 W1 결과 절), `docs/IMPLEMENTATION_NOTES.md`.

### 2026-07-28 · [test] · 완료 · FE-MULTI 단언형 E2E — 실제 브라우저에서 v2 계약 검증
- **요청(사용자)**: 개선안 4번.
- **동기**: 기존 `demo-scenario.mjs` 는 **녹화 스크립트**로 단언이 하나도 없어 화면이 조용히
  망가져도 통과한다. v2 의 핵심 주장(동시 스트리밍·1:1 귀속)을 검증하는 테스트가 없었다.
- **결과**: `frontend/e2e/concurrent-turns.assert.mjs` — 서버(모의 LLM)·vite 를 직접 띄우고
  브라우저 컨텍스트 2개로 두 게스트를 만들어 **실제 DOM** 에서 9개 단언. 실패 시 exit 1.
- **⚠️ 첫 버전은 위양성이었다(정직히 기록)**: 처음엔 "활성 턴 배지가 2 에 도달" + "답글 2개 존재 +
  ↳@ 라벨"로 병렬을 판정했다. **음성 대조군(`E2E_CONCURRENT=0`, 직렬 계약)을 돌려보니 그대로 통과**했다.
  원인: ① `turn.ts` 가 턴 시작 시 `activeTurnCount+1` 로 RUNNING 을 publish 하므로 직렬에서도
  디스패치 순간 배지가 2 가 된다(뒤 턴은 곧 큐 대기) ② AGENT_REPLY 행과 귀속 라벨도 턴 시작 시
  생성되므로 직렬에서도 둘 다 존재한다. **즉 배지·라벨은 병렬의 증거가 아니다.**
- **수정 — 진짜 분별자로 교체**: 각 답글 버블의 본문 길이를 60ms 간격으로 시계열 샘플링해
  **두 답글이 시간상 겹쳐 자라는지**를 판정한다(`min(lastGrow) - max(first) > 0`).
  - 병렬(v2): 겹침 양수 + 동시 성장 샘플 > 0 → **PASS**
  - 직렬(v0.1): `qa` 391→1759ms 스트리밍 후 `qb` 가 2036ms 에 시작 → **겹침 −277ms, 동시 성장 0회 → FAIL**
  - 음성 대조군이 실패하는 것을 확인해 **테스트가 실제로 분별함을 증명**했다.
- **flaky 방지 설계**: LLM 은 모의 서버만(실 LLM 지터는 겹침 판정을 불안정하게 만든다).
  인증·글생성·전송은 API 로(Composer AI 토글은 과거 데모 촬영에서 재시도가 필요했던 flaky 지점 —
  검증 대상과 무관한 위험을 테스트에 들이지 않는다). **검증은 전부 실제 DOM**.
- 변경 파일: `frontend/e2e/concurrent-turns.assert.mjs`(신규), `frontend/package.json`, `README.md`.

### 2026-07-28 · [chore] · 완료 · HOL 대비 20초 클립 — 발표용 핵심 시각물
- **요청(사용자)**: 개선안 3번.
- **결과**: `docs/assets/hol-clip.html`(CRT 팔레트 계승) + `frontend/e2e/render-hol-clip.mjs`
  (Playwright `recordVideo` 로 녹화 → ffmpeg H.264 변환) → `docs/assets/hol-clip.mp4`(1280×720, 19초, 777 KiB).
- **왜 실촬영이 아니라 데이터 기반 생성인가**: 두 계약을 실제 서버로 동시 촬영하면 실행마다 지연이
  흔들려 **클립 숫자와 논문 표가 어긋난다** — 그러면 오히려 신뢰를 깎는다. `EXPERIMENTS §E2` 실측
  상수(L=15s, 직렬 14930ms, 병렬 235ms)에서 결정적으로 생성해 **항상 같은 그림**을 얻는다.
  임의 연출 값은 없다(표시용 스트림 길이 1.4s 만 고정).
- **핵심 장면**: 왼쪽 "바다가 기다린 시간" 카운터가 실시간으로 **14.93s** 까지 올라가는 동안
  오른쪽은 **0.23s** 에 멈춘 채 응답을 완료한다. 1:1 실시간이라 시청자가 대기를 **체감**한다.
- `.gitignore` 의 `*.mp4` 에 예외를 뒀다(<1MB · 재생성 가능 · 심사 핵심 시각물).
- 변경 파일: `docs/assets/hol-clip.{html,mp4}`, `frontend/e2e/render-hol-clip.mjs`,
  `frontend/package.json`, `.gitignore`, `README.md`.

### 2026-07-28 · [fix] · 완료 · 데모 스크립트의 미선언 의존성 — `@playwright/test` 누락
- **발견 경위**: 개선안 4번(프론트 단언형 E2E) 착수 중 확인.
- **결함**: `frontend/e2e/demo-scenario.mjs:7` 이 `@playwright/test` 를 import 하는데
  **어느 `package.json` 에도 선언돼 있지 않다.** 리포 어디에도 설치돼 있지 않아
  `ERR_MODULE_NOT_FOUND` 로 즉시 실패한다 — 즉 **클린 체크아웃에서 데모 녹화가 불가능**하다.
  지금까지는 개발 머신에 남아 있던 설치본에 의존해 돌았던 것으로 보인다.
- **수정**: `frontend/package.json` 에 devDependency 로 선언 + README 검증 절에 브라우저 설치 안내.
- 변경 예정 파일: `frontend/package.json`, `README.md`.

### 2026-07-28 · [feat] · 완료(단, **성능 이득 0** — 아래 실측) · XC-SCOPE — 부수효과 락을 샌드박스 단위 → **충돌 단위(파일)** 로 세분화
- **요청(사용자)**: "락 파일 단위로 좁히는 것부터 진행해줘".
- **동기(E2-B 실측)**: 두 동시 턴이 모두 파일을 수정하면 병렬 이점이 **1.07×까지 붕괴**한다.
  원인은 `withSandboxLock(session.sandboxId, …)` 이 **샌드박스 단위 단일 mutex** 라서
  **서로 다른 파일**을 만지는 턴도 줄을 서기 때문이다. TRD §6.4가 이미 한계로 적어둔 항목이다
  ("필요 시 같은 파일을 건드리는 턴만 직렬로 좁히는 폴백 가능").
- **핵심 설계 판단 — 단순 mutex 가 아니라 계층 락이어야 한다**:
  `SHELL`/`PACKAGE` 는 **어떤 파일을 만질지 알 수 없다**(파이프·변수전개·서브셸). 따라서 파일 단위로
  좁힐 수 없고 **샌드박스 전체 배타**여야 한다. 결국 필요한 것은 다음 호환성 행렬이다:

  | | FILE_WRITE/READ(경로 P) | FILE_WRITE/READ(경로 Q≠P) | FILE_DELETE | SHELL/PACKAGE |
  |---|---|---|---|---|
  | **FILE_WRITE/READ(P)** | 직렬 | **병렬 ✅(이번 이득)** | 직렬 | 직렬 |
  | **FILE_DELETE** | 직렬 | 직렬 | 직렬 | 직렬 |
  | **SHELL/PACKAGE** | 직렬 | 직렬 | 직렬 | 직렬 |

  - `FILE_DELETE` 를 **배타로 둔 이유(안전 우선)**: 삭제는 디렉토리를 지울 수 있어
    `rm -r src/` 와 `write src/x.py` 가 경로 키가 달라 병렬 진입하면 **ENOENT 레이스**가 새로 생긴다.
    삭제는 드물어 배타로 둬도 비용이 거의 없다. **새 레이스 클래스를 만들지 않는 것**이 우선.
  - `OTHER`/미지 kind 도 **배타**(fail-safe).
- **구현 방침(`sandboxLock.ts`)**: 샌드박스별 **FIFO 대기열 + 실행 중 집합**을 두고, 큐 머리에서
  현재 실행 중인 작업들과 호환되는 동안만 입장시킨다.
  - **엄격 FIFO(추월 금지)**: 머리에서 막히면 뒤도 대기한다 → `SHELL` 이 파일 작업 스트림에
    **굶지 않는다**(starvation 방지). 동시성을 조금 포기하고 공정성을 택했다.
  - 경로 키 정규화: `path.normalize` + 구분자 통일 + **win32 는 소문자화**(대소문자 무구분 FS 라
    `A.txt`/`a.txt` 가 같은 파일이므로 같은 키여야 한다).
  - 기존 `withSandboxLock(key, fn)` 은 **`{mode:'exclusive'}` 별칭으로 보존** — 기존 호출부·테스트
    바이트 동일 동작(전부 배타 → 오늘과 같은 완전 직렬).
- **롤백 스위치 + 실험 변인**: `LOCK_SCOPE=file|sandbox`(기본 `file`). `sandbox` 면 이전 동작으로
  즉시 되돌아간다. 이 플래그가 E2-B 재측정의 **조작 변인**이 되어 개선폭을 수치화한다.
- **문서 정합 후속(미처리로 남김)**: `PATENT.html`/`PAPER.html` 은 "부수효과를 **샌드박스 단위**
  단일 직렬 실행기로 적용"이라고 서술한다. 이 변경은 그것을 "**충돌하는** 부수효과를 직렬화"로
  일반화한다(안전 보장은 동일, 입도만 세분화). 청구항 문구 조정은 별도 판단이 필요하므로
  이번 커밋 범위에 넣지 않고 **불일치 사실만 기록**한다.
- **검증 계획**:
  1. 신규 `test/sandboxLockScope.test.ts` — 호환성 행렬 전수(같은 경로 직렬 / 다른 경로 병렬 /
     SHELL 배타 / DELETE 배타 / FIFO 굶주림 없음 / reject 가 체인 안 끊음 / 키 정리 / win32 대소문자).
  2. 기존 `sandboxLock.test.ts` **무수정 통과**(하위호환 증명).
  3. `LOCK_SCOPE=sandbox` vs `file` 로 **E2-B 재측정** → 1.07× 가 얼마로 개선되는지 실측.
  4. 전체 회귀(백/프론트 스위트·typecheck·빌드·keygate).
- **검증 결과 — 구현·안전성은 통과, 그런데 성능 이득이 0이다(정직히 기록)**:
  - 신규 `test/sandboxLockScope.test.ts` **13건 통과** — 호환성 행렬 전수(다른 경로 병렬 8개 동시,
    같은 경로 직렬, exclusive↔path 양방향 배타, exclusive끼리 직렬, **대기 중 exclusive 추월 금지**,
    경로 정규화 `./`·`\`·win32 대소문자, reject 가 체인 안 끊음, 키 정리).
  - 기존 `test/sandboxLock.test.ts` **무수정 통과** — `withSandboxLock` 을 exclusive 별칭으로 보존해
    하위호환 증명.
  - **조작 변인 `LOCK_SCOPE=sandbox|file` 실측(나머지 조건 동일)**:

    | 시나리오 | sandbox | file | 차이 |
    |---|---|---|---|
    | E2-B: A가 SHELL 6s + B도 도구 | 8206ms | 8212ms | **0%** |
    | 서로 **다른 파일**에 각 40회 write | 1001ms | 980ms | 2%(노이즈) |
    | **같은 파일**에 각 40회 write(대조) | — | 1009ms | — |

  - **원인 규명**: ① E2-B 붕괴는 A의 **SHELL 이 락을 6초 보유**한 탓이고 SHELL 은 어느 설정에서도
    배타여야 하므로(어떤 파일을 만질지 알 수 없다) 입도로는 고칠 수 없다 — 두 설정 차이 6ms가 이를 확정.
    ② 파일 쓰기 경합은 **애초에 미미**했다. `write_file` 의 락 보유는 ms 급인데 쓰기 1회 비용은
    워커↔부모 ack + **새 LLM completion** 이 지배한다. 같은 파일(1009ms)과 다른 파일(980ms)이
    사실상 같다는 것이 "직렬화 여부가 결과를 바꾸지 못하는 경합 수준"이었음을 보여준다.
  - **그래도 유지하는 이유(성능이 아니라 보험)**: 안전 보장 동일 + 입도만 세분화 + 즉시 롤백 가능이며,
    **컨테이너-per-도구호출**(부록 B 계획, 호출당 200~500ms)을 도입하면 락 보유가 지배적이 되어
    실질 이득이 난다 — 격리 강화의 **선행 조건**이다.
  - **E2-B 개선에 필요한 다른 레버(범위 밖)**: SHELL 의 부수효과 범위를 사전 신고하게 하거나,
    턴별 샌드박스 분기 + 쓰기 충돌만 병합. 둘 다 설계 재검토가 필요하다.
- **하네스 추가**: `mockLlm.mjs` 에 `fwrite=N fpath=…` 지시자(같은 경로에 N회 write_file 루프 —
  convo 히스토리의 tool 결과 수를 세어 결정적으로 진행). `e2-hol.mjs` 에 `FWRITES`/`SAME_FILE`/
  `LOCK_SCOPE` 전달 추가.
- **회귀**: 백엔드 **36 파일 / 152 테스트** 통과, typecheck·keygate 통과,
  프론트 9 파일 / 127 통과 + 빌드 통과.
- 변경 파일: `backend/src/agent/sandboxLock.ts`, `backend/src/agent/turn.ts`,
  `backend/src/config.ts`, `backend/test/sandboxLockScope.test.ts`,
  `backend/bench/{mockLlm,e2-hol}.mjs`, `docs/TRD.md`(§6.4), `docs/EXPERIMENTS.md`(XC-SCOPE 절),
  `docs/IMPLEMENTATION_NOTES.md`.

### 2026-07-28 · [test] · 완료 · E2-B(락 경합 하 TTFT) + 실 LLM 모드 — 하네스 LLM 비의존화
- **요청(사용자)**: 개선안 2번(두 턴 모두 도구를 쓰는 E2 변형)을 **실 LLM으로** 진행. 실패 시 mock 폴백.
- **동기(내가 스스로 지적한 약점 닫기)**: 기존 E2 실측은 B가 도구를 쓰지 않아 샌드박스 락을
  다투지 않는 **행복 경로**였다. 두 턴이 모두 부수효과를 낼 때도 "추론은 병렬"이 유지되는지는
  미검증이었고, 그게 실사용에서 더 흔한 시나리오다.
- **측정 설계 — 계약의 두 절반을 분리한다**:
  - `B TTFT`(첫 `agent.token`) = **추론이 병렬인가**. 락과 무관해야 한다.
  - `B 완료 시각`(`message.updated` → COMPLETE) = **부수효과가 직렬인가**. A의 긴 도구 뒤에서
    락을 기다리므로 늘어나야 한다.
  - `A 완료 시각` = 간섭 비용(B 때문에 A가 느려지지 않는지).
  - 두 지표가 **갈라지는 것**이 v2 계약의 실증이다(TTFT 불변 + 완료 지연). 하나로 뭉치면 못 본다.
- **하네스 변경(`bench/e2-hol.mjs`)**:
  - **턴 식별을 `replyToId` 기반으로 교체**. 기존에는 모의 LLM이 첫 토큰에 심은 `[id]` 마커로
    턴을 구분했는데, 이는 mock 전용이라 실 LLM에서 동작하지 않는다. `message.created` 가
    `replyToId`(= 그 턴을 유발한 HUMAN 메시지 id)를 실어주므로 이걸로 AGENT_REPLY ↔ 사용자를
    1:1 귀속한다 — **레거시·병렬 경로 모두, mock·실 LLM 모두** 동작한다(하네스의 LLM 비의존화).
  - `LLM=real|mock`(기본 mock): real 이면 모의 서버를 띄우지 않고 서버 `.env` 의 실 키를 쓴다.
  - `BOTH_TOOLS=1`: B도 도구를 실행해 락을 다투게 한다(E2-B 시나리오).
  - 실 LLM 모드 발화는 **명령을 명시**한다(모델이 지연 길이를 통제할 수 없으므로
    "이 bash 명령을 그대로 실행해줘" 형태로 sleep 길이를 지시).
- **정직한 사전 기록 — 실 LLM의 측정 한계**: 버스트 프로브에서 첫 토큰 지연이 **828~2005ms**로
  관측됐다(±1.2s). L=2s 구간에서는 신호의 60%에 해당하는 잡음이라, 실 LLM 결과는 mock 결과보다
  분산이 크고 조건 간 분리가 흐려질 수 있다. 또 모델이 지시한 명령을 그대로 실행하지 않으면
  L 자체가 통제되지 않는다. **이 두 위험이 실측에서 확인되면 그 사실을 결과로 보고하고**
  통계용으로는 mock 결과를 정본으로 둔다(사용자 사전 승인: "실패하면 그때 목업").
- **쿼터 보호**: 지난번 일일 한도 소진(retry-after≈7.4h) 재발을 막기 위해 **소규모 파일럿 먼저**
  돌리고(셀당 3런), 성공률·분산·소모율을 보고한 뒤 확대 여부를 결정한다.
  결과 파일은 `OUT_TAG` 로 분리해 커밋된 180행 정본을 덮어쓰지 않는다.
- **실측 결과 — 내가 스스로 적어둔 "행복 경로" 약점이 정확히 얼마인지 드러났다**:

  | 시나리오 | C-FIFO | C-PAR | 개선 |
  |---|---|---|---|
  | B가 추론만(원 E2, L=15s, n=20) | 14930ms | **235ms** | **63.5×** |
  | B도 도구 사용 · mock(L=6s, n=3) | 8262ms | **7735ms** | **1.07×** |
  | B도 도구 사용 · 실 LLM(L=6s, n=2) | 14341ms | **10624ms** | **1.35×** |

  - **두 동시 턴이 모두 부수효과를 내면 직렬 실행기가 병목이 되어 이점이 1.07×까지 붕괴한다.**
    설계 결함이 아니라 부수효과 직렬화를 **선택**한 계약의 필연적 귀결이다(대가: 파일 깨짐 0·머지 불필요·1:1 귀속).
  - **실 LLM(1.35×)이 mock(1.07×)보다 이점이 크다** — 실 추론은 호출당 수 초가 걸리고 그 부분은
    실제로 병렬화된다. mock은 추론을 200ms로 주입했으므로 병렬화할 몫이 거의 없었다.
    즉 실 LLM 사용이 유익했다: mock만 봤으면 "이점 없음"으로 오결론했을 것이다.
  - `bTtftMs`와 `bCompleteMs`가 20ms 차로 붕괴 — gpt-4o-mini가 **서두 텍스트 없이 곧바로
    `tool_call`을 방출**하므로 이 시나리오의 "첫 토큰"은 추론 시작이 아니라 도구 왕복 이후를 잰다.
    설계했던 두 지표 분리가 이 모델에서는 성립하지 않는다(모델·프롬프트 의존 — 문서에 명시).
  - **실 LLM 실행 성공률 6/6, 쿼터 실패 0.** 버스트 프로브 4/4 통과 후 실행.
- **주장 범위 정정(README·BUSINESS_VALUE·EXPERIMENTS 3곳)**: "수십 배"는 **동시 질문이 추론 중심일 때**
  (설명·리뷰·질의응답)로 한정하고, 모두 파일을 수정하는 경우 1.07~1.35× 임을 명시했다.
  세일즈에서 "동시에 다 같이 파일을 고친다"로 팔면 실측과 어긋난다는 경고도 BUSINESS_VALUE에 기재.
- **하네스 회귀 확인**: `replyToId` 귀속으로 교체한 뒤 mock 스모크가 **기존과 동일 수치 재현**
  (C-FIFO 1947 / C-PAR 236 / C-REJECT 2258 vs 이전 1925 / 238 / 2258). 정본 180행 파일 무영향(OUT_TAG 분리).
- **후속(미실시)**: 같은 파일 vs 다른 파일 구분(락을 파일 단위로 좁히면 서로 다른 파일을 만지는 턴은
  병렬화 가능), B 도구 길이 스윕, 실 LLM n 확대(현재 n=2).
- 변경 파일: `backend/bench/e2-hol.mjs`, `backend/bench/out/e2-hol.{mockboth,realpilot}.jsonl`(미추적),
  `docs/EXPERIMENTS.md`(E2-B 절 신설), `README.md`, `docs/BUSINESS_VALUE.md`, `docs/IMPLEMENTATION_NOTES.md`.
...
- **문서**: `docs/DEMO_SCENARIO.md` §1 창 배치를 작업영역 타일링으로 수정(화면 꽉 채움).
...
- **조사 핵심 결론**: 본 발명의 특정 결합(단일 공유 세션·convo·cwd + 병렬 턴 추론 + 부수효과 비관적 직렬화)을 단독 개시하는 문헌 **부재** — 쟁점은 진보성(조합 용이성). 최근접 Top 5 = US 12,346,664 B1(OpenAI, 장기 작업 중 병렬 프롬프트) / CoAgent(arXiv:2606.15376, 추론 병렬+쓰기 순서화이나 낙관·수리형 saga undo, 잠금 부적합 교시=교시회피 자산) / US 2024/0430216 A1(MS, 다중 사용자 코파일럿 세션·순차 처리) / DeLM(arXiv:2606.10662, admission만 직렬) / Replit Agent 멀티플레이어(격리 사본+사후 머지). 기존 인용 검증: US 11,330,026 B1·US 9,626,509 B1·MPAC·Jules·Qyclaw·Frona·MLflow 실존 확인(유지·비중 조정), **"sBot"→"Spacebot" 표기 정정**, 미실존 판정 0건.
- **PATENT.html 개정 내역**: ① 배경기술 재편(나~바: 공유 세션 공지와 직렬성 한계 / "공유 컨텍스트면 직렬화" 통상 상식=교시회피 / 격리 후 병합(낙관) 계열 / 에이전트 동시성 제어 연구 / HOL 미해결 과제) + 이전 세션 개정 시 중복 잔존한 구 '라. 머리-막힘' 블록(구 [0006]~[0008]) 제거. ② 【선행기술문헌】 전면 교체 — 특허문헌 10건(MS·OpenAI·Airia·Google·Baidu·Kakao·Samsung 신규 7건 + 기존 2건 + 모출원), 비특허문헌 24건(CoAgent·DeLM·Atomix·Cordon·Harness-MU·TianPan·TD Commons·Kung&Robinson·langchain-k8s·LangGraph·Replit·ChatGPT 그룹챗·Zed 등 실존 검증 인용). ③ 【해결하려는 과제】에 3중 정식화(응답성·작업공간 정합성·컨텍스트 일관성 — 종래는 최대 둘) 추가. ④ §10 구별 논리를 2축→3축(추론 직렬화·배칭 대신 병렬 / 격리 후 병합 대신 공유·무병합 / 사후 검출·수리 대신 미연 방지)으로 재작성 + 표 2를 재조사 문헌 기준 11행으로 교체. ⑤ 청구범위 18항→17항 재작성 — 독립항 1에 무머지·완료순 한정 필수 기재, 스냅샷+원자 블록 커밋을 2항(최우선 사수)으로 승격, 무OCC·무롤백·last-wins 명시항(3항) 신설, 구 6항(카운트)+14항(suspend/resume) 병합(8항), 구 7항(공정 큐)+9항(per-user 1) 병합(7항), turnId 태깅 fan-out+재생(10항)·1:1:1 결속(11항) 신설, 방법 독립항(14항)에 "시간적으로 중첩"·"상호 배타적 완료순" 문언 명시.
- **검증(③) — 실측**: HTML 파서 태그 밸런스 **미닫힘 0·mismatch 0**, 문단 번호 [0001]~[0041] **41개 단조·중복 0**, 청구항 1~17 연번 정상, 본문 인용 번호가 문헌 목록과 완전 정합(특허문헌 1–10, 비특허문헌 1–24, 고아 참조 0), 구 블록·"sBot"·구 청구항 18 잔존 **0건**.
- **절차 메모(보고서 제6.4장)**: CoAgent(6/13)·Cordon(6/16)·Harness-MU(6/20)·TD Commons(6/29)는 2026-06 공개 — 조속한 출원으로 선행기술 지위 배제 가능. 자사 서비스 공개 여부에 따라 공지예외주장(특허법 §30, 12개월) 준비 권고.
- **개정(同日, 사용자 요청) — CopilotKit/OpenTag 차별성 검토·반영**: 사용자 제보 유사 기술 github.com/CopilotKit/OpenTag(Slack 등 그룹 채팅 내 자가호스팅 AI 에이전트 봇, CopilotKit `@copilotkit/bot` 기반) 실조사. 공식 문서 "Threads & Persistence Architecture" 확인 결과 **스레드당 단일 활성 run을 Redis 분산 락(lock TTL 20s·하트비트)으로 강제, 동시 run은 409 Conflict 거부**("두 run이 동일 스레드 이벤트를 인터리브하지 못하도록") — 본 발명 배경기술이 종래기술로 지적한 **바쁨(busy) 게이트 직렬화의 전형**이며, 실행 샌드박스·작업 디렉토리·파일 부수효과 계층 자체가 부재(도구 호출·리치 렌더링만). **판정: 차별성 명확(특허 진행 가능)** — 락의 목적이 병렬 실행 차단인 반면 본 발명의 락(133)은 병렬 추론을 유지한 채 부수효과만 직렬화. 오히려 "공유 스레드면 직렬화·거부" 통상 상식의 상용 실시례로서 교시회피 논거를 강화 → 비특허문헌 25로 추가, 배경기술 다(多) 문단·과제 [0009]·구별 [0041] 축1·표 2에 반영.
- 변경 파일: `docs/PATENT.html`, `docs/IMPLEMENTATION_NOTES.md`.
...
- **산출물**: `docs/paper-acm/acmart-primary/paper.tex`(약 50KB) + `paper.bib`(16 entries). acmart.cls/.bst와 동일 폴더에 배치(로컬 컴파일 가능) — Overleaf 업로드 시 paper.tex+paper.bib만으로도 충분(acmart 내장).
- **변환 매핑**: ① 마스트헤드→`\title[short]{...}`+`\author/\affiliation/\email`+`\renewcommand{\shortauthors}`. ② Abstract→`\begin{abstract}`, Index Terms→`\keywords{...}` + **CCS 개념 4건**(`CCSXML`+`\ccsdesc`, 제출 전 dl.acm.org/ccs 재생성 주석). ③ §I–IX→`\section/\subsection` + `\label/\ref`(섹션·그림·표 상호참조 전부 번호 자동화: §IV-A 등 9개 라벨). ④ **SVG 도면 2개→네이티브 TikZ `figure*`**(외부 이미지 0, pdflatex 직컴파일; 도1 아키텍처 3패널·도2 병렬추론+직렬부수효과 시퀀스, `\Description{}` 접근성 포함). ⑤ 표 2개→`booktabs`(Table I 단단, Table II `table*` 풀폭). ⑥ 인라인 `[n]`→**BibTeX `\cite`** + `\bibliographystyle{ACM-Reference-Format}`(16참고문헌 .bib화). ⑦ 특수문자 LaTeX 이스케이프(`\_`/`\&`/`\{\}`/`\textless`/`$\times$`/`$\leftrightarrow$` 등), 글리프(◉/↳)는 텍스트로 대체.
- **편집 필요 플레이스홀더(템플릿 관행 유지)**: copyright/conference/ISBN/DOI(연도만 2026으로), CCS 개념(재생성 권장). 저자 이메일은 실주소 확정분.
- **검증(③) — 정적(로컬 TeX 미설치=컴파일 불가, Docker만 존재하나 멀티-GB 이미지 풀은 무단 미실행)**: 파이썬 정적 검사 **전부 통과** — 중괄호 458/458 균형, `\begin`/`\end` 환경 스택 잔여 0·문제 0, 인라인 수학 `$` 152(짝수), `\cite` 16키 ↔ `.bib` 16키 **완전 일치(고아 0)**, 모든 `\ref`(15)→`\label`(17) 해소·중복 라벨 0, 주석/수학/CCSXML 제외 미이스케이프 `_`/`#` **0건**. → 컴파일 차단성 오류 없음 확신(잔여 위험은 TikZ 미세 배치·float 경고 수준).
- 변경 파일: `docs/paper-acm/acmart-primary/paper.tex`(신규), `docs/paper-acm/acmart-primary/paper.bib`(신규), `docs/IMPLEMENTATION_NOTES.md`. (※ `docs/paper-acm/`에 acmart 배포본 전체가 해제돼 있음 — 컴파일엔 `acmart.cls`+`ACM-Reference-Format.bst`만 필요, 나머지(samples/·*.pdf·*.dtx)는 삭제 가능.)

### 2026-06-30 · [docs] · 완료 · IEEE 논문 초안(영문) — `docs/PAPER.html` 신규(병렬 추론 + 직렬 부수효과)
- **요청(사용자)**: docs 문서들(PRD/TRD/PLAN/IMPLEMENTATION_NOTES/PATENT)을 토대로 **IEEE 형식 논문을 HTML로** 작성. 문제 프레이밍은 **정보 불일치(information inconsistency)** — (i) AI 이전: 중간관리자-실무자 사일로로 실무자가 A 대신 B를 함, (ii) AI 이후: 1인 1 code-agent 세션이라 컨텍스트가 분기, 공유 메모리(personal/company-wide `memory.store`)도 **실시간 공유 안 됨 + 사람 매개 전파(병목)** 라 동시 작업에서 불일치 잔존, (iii) 해결: **단일 공유 세션**(전원 attach → 단일 권위 컨텍스트 → 불일치 구조적 제거). 단일 공유 세션의 직렬화가 부르는 **HOL blocking**은 **병렬 추론 + 직렬 부수효과**로 제거.
- **결정(사용자)**: 본문 언어 = **영문**(IEEE 관행), 저자 = **Wondeuk Yoon · Dujeong Lee(삼성전자)**, 이메일/소속은 제출 전 교체용 **플레이스홀더**.
- **범위**: `docs/PAPER.html` 신규 1개만 — 자기완결 HTML(외부 의존 0·JS 0·인쇄→PDF 친화), `PATENT.html`과 동일 thesis를 **학술 논문 형식**(Abstract / Index Terms / §I–IX / References)으로 재구성. **코드·스키마·런타임 무변경**(소스 변경 0). `PATENT.html`은 보존(별개 산출물).
- **구성**: §I 서론(조직적 동기=정보 불일치), §II 동기(3 regime 표·메모리 한계·HOL blocking), §III 시스템 개요(서버키 모델·단일 생성 fan-out·seq SoT), §IV 동시성 계약(독립 턴 병렬 추론·샌드박스 단위 직렬 실행기·convo 스냅샷+완료순 직렬 커밋·turnId 멀티플렉싱/턴 지정 인터럽트·활성턴 카운트 상태·`MAX_CONCURRENT_TURNS`+공정 큐·1인1활성턴·opt-in 게이트·대안 기각), §V 구현(`piWorker.mjs`/`pi.ts`/`turn.ts`), §VI 평가(실측 E2E), §VII 관련연구(공유 vs 격리 · 예방 vs 검출 2축), §VIII 한계, §IX 결론, 참고문헌 16건. **도 1**(아키텍처)·**도 2**(병렬추론+직렬부수효과 시퀀스) 인라인 SVG.
- **수치 출처**: 평가 표/본문은 본 changelog의 **AR-MUX**(concurrent ON 토큰 인터리브·첫토큰 ~동시·완료 92ms / OFF 직렬·완료 3.3s차), **XC-CAP**(maxInflight 1/2/4 = 1인1턴·사용자간 병렬·cap=4), **RT-MULTI**(activeTurns), **FE-MULTI**(라이브 브라우저 E2E activeTurns=2 배지·구분띠·귀속·내턴 게이팅) 실측값을 인용. PoC 단일 런·예시값임을 본문에 명시.
- **검증(③) — 실측**: ① HTML 태그 밸런스 파서(`html.parser`) **잔여 미닫힘 0(균형)** · SVG 2 · figure 2 · table 2. ② 로컬 HTTP(`127.0.0.1:8000`) + Chrome 렌더 실측 — **2단 IEEE 레이아웃·풀폭 마스트헤드·Abstract/Index Terms·도1/도2 SVG 풀폭 스팬·Table I(컬럼 내)·Table II(풀폭 스팬)·References 2단** 모두 정상 표시 확인. ③ JS 없음 → 콘솔 오류 불가. (초기 Table II가 `overflow` 래퍼 안 inner `column-span`이라 미스팬 → 래퍼(`.tbl-wrap.span`)로 격상 후 풀폭 확인.)
- **개정(同日, 사용자 요청)**: §VII 관련연구에 전용 단락 + §IV-H에 미러로 **멀티유저 쿼리/툴콜 배칭** 선행 접근과 그 **쿼리 오염(query pollution)** 2모드 추가 — ① **교차 귀속 오염(cross-request contamination)**: 서로 다른 사용자 요청을 한 추론에 섞어 응답·툴콜이 엉뚱한 사용자에 귀속(1:1 붕괴·무요청 액션), ② **결함 격리 상실(loss of fault isolation)**: 악의적/오염 입력 1건(예: 프롬프트 인젝션)이 공유 배치 턴 전체를 망침/실패시켜 무관한 사용자 출력까지 오염. 독립 턴(배칭 0)이 이를 구조적으로 회피 → **per-turn 결함 격리**를 본 설계의 이점으로 명시(§IV-H 효과 목록·§IV-A 상호참조). 인라인 텍스트/단락 분할만(구조 무변경), HTML 태그 밸런스 재검 **잔여 미닫힘 0** + Chrome 재렌더로 §VII 새 단락 2단 흐름 정상 확인.
- **개정 2(同日, 사용자 요청)**: ① **fault isolation 승격** — Abstract에 "독립 턴=비배칭 → 악성/오염 입력이 자기 턴 내에서만 실패" 1문장 + 기여 목록 ④ 평가 항목에 `per-turn fault isolation under adversarial input` 추가. ② **§VI-E "Fault isolation under adversarial or failing input" 신설 + Table II에 S4 행**("악성/실패 턴 + 정상 턴 동시"): 비교차 라우팅·turn 지정 인터럽트(자기 턴만 실패·이웃 COMPLETE)·`{callId,turnId}` 교차 ack 무오류는 **`arMux.test.ts` 실증**, 악성 입력 격리는 **by-construction 귀결로 명시(허위 측정치 미생성)**. Table II 캡션을 S0–S3=라이브 단일런 / S4=런타임 멀티플렉싱 테스트 하버스 출처로 정정. ③ **마스트헤드 Correspondence 정리** — '이메일만' 남기고(`{wondeuk.yoon, dujeong.lee}@samsung.com`, 사용자 실주소 확인) 'placeholder affiliation/contact — edit before submission' 캐비엇 제거 + 말미 note의 "Author and affiliation fields are placeholders" 문장 삭제(저자·이메일·소속 모두 확정). 검증: 태그 밸런스 재검 **잔여 미닫힘 0** + Chrome 재렌더로 S4 행/§VI-E/정리된 마스트헤드 정상 확인.
- 변경 파일: `docs/PAPER.html`(신규), `docs/IMPLEMENTATION_NOTES.md`.

### 2026-06-30 · [feat] · 완료 · M8 FE-MULTI — 프론트 다중 스트리밍 UI(1:1 귀속 · 동시 턴 배지/구분 띠 · Composer 내턴 게이팅 · reduced-motion)
- **요청(사용자)**: RT-MULTI에 이어 M8 **마지막 WP**, workflow로 착수. 백엔드 v2(XC-MODE/XC-SERIAL/AR-PAR/AR-MUX/XC-CAP/RT-MULTI)가 완료돼 동시 다중 턴 병렬 추론·직렬 부수효과·활성 턴 카운트가 백엔드에서 표면화된 상태 → 이제 그 동시 다중 턴을 **순수 프론트**에서 사용자에게 표면화한다.
- **범위(FE-MULTI only)**: ① `session.status.activeTurns` 소비 + 헤더 배지(`◉ N개 작업 진행 중`, term-amber; ≥1 표시·≥2 강조). ② `replyToId` 1:1 귀속 — 각 AGENT_REPLY 상단 `↳ @<질문자>` 마이크로라벨(replyToId→HUMAN 메시지→authorName), **동시 2턴+ 스트리밍 시 term-amber-line 칩으로 위계 승격**, **본인 질문 답글**(그 HUMAN authorId===내 userId)은 좌보더 term-active + 탭 시 대응 HUMAN 버블로 스크롤 앵커(양방향). ③ 동시 스트리밍 2턴+ 구분 띠(`── 동시 스트리밍 중 (N턴) ──`, term-dim). ④ Composer **'내 턴' 게이팅(self-concurrency=1)** — 내 활성 턴 inflight 동안만 내 Composer 잠금 + `thread.myTurnBusy` 안내, **남의 활성 턴은 비차단**(HOL blocking 제거). ⑤ TOOL_CALL/TOOL_RESULT 소속 턴 `↳@<질문자>` 귀속(단일 턴이면 생략). ⑥ prefers-reduced-motion — 동시 N개일수록 점/스파클 애니메이션 정지 → 정적 `작성 중` 텍스트. **term-*·i18n only(새 색 도입 금지), 단일 턴(레거시/비동시)은 오늘 UX와 동치**.
- **범위 밖**: 백엔드 무변경(RT-MULTI가 activeTurns 이미 송신). 워크스페이스 파일 트리 패널(별개). concurrent+steer 인터럽트. 고밀도 처리(동시 다수 턴 접기→1줄 요약)는 **최소·후속 가능**(우선순위 낮음, 빠지면 문서에 후속 명시).
- **구현(요지)**: 스트리밍 누적은 **이미 다중 지원**(`appendToken`이 messageId별 누적, ChatBubble이 각자 STREAMING 시 자기 `.term-cursor` 표시) → 스트림/스토어 누적 로직 무변경. FE-MULTI는 주로 **귀속/배지/게이팅/구분 띠/감속모드의 additive 표면화**. 순수 로직(귀속 resolve, 내턴 active 셀렉터, activeTurns 소비)은 헬퍼/셀렉터로 분리해 단위 테스트로 검증. `useThreadStream`이 `session.status`의 `activeTurns`를 스토어에 반영, `threadStore`에 activeTurns 보관 + '내 활성 턴' 셀렉터, Composer가 그 셀렉터로 게이팅.
- **구현(실측)**: 신규 순수 셀렉터 `frontend/src/lib/threadSelectors.ts`(store/React/DOM 비의존) — `countActiveStreamingTurns`/`resolveActiveTurnsCount`(배지만 서버 권위 `activeTurns`>0 우선, 그 외 messages 파생=SSE 지연 강건)/`hasMyActiveTurn`(replyToId→HUMAN.authorId===내 userId 체인; 남의 턴은 false=HOL blocking 제거, 대상 HUMAN 미수신 시 과소잠금=Composer `sending` 커버)/`buildAttribution`(self=username||you·작성자=postAuthorName·그 외 peer=someoneElse 폴백, **빈 라벨 금지**; anchorHumanId 로 양방향 앵커)/`attributeToolBubble`(TOOL_*는 replyToId 없음 → seq-인접 직전 AGENT_REPLY 상속, 단일 턴 정확·동시 근사). `threadStore.ts` — `activeSessionTurns:number`(init/reset 0) + 별도 `setActiveSessionTurns`(기존 `setSessionStatus(status)` 호출부 무변경, 누적 로직 전부 불변). `useThreadStream.ts` — `session.status` 핸들러가 `if(typeof p?.activeTurns==='number') setActiveSessionTurns()`(레거시 미수신→미호출→0 유지), deps 추가. `ChatBubble.tsx` — props `attribution?`/`concurrent?`/`onAnchorJump?`; AGENT_REPLY 상단 `↳ @<질문자>` 라벨(**단일 턴=isMine 일 때만, 동시=항상** 노출 → 단일 턴 픽셀 동치), 동시 시 term-amber-line 박스 칩 위계 승격, isMine 시 좌보더 `border-l-2 border-term-active pl-2` + 라벨 button 앵커, PENDING(빈 본문) 시 `✦ 작성 중 •••`(`.term-dots`)·그 외 STREAMING 시 기존 `.term-cursor`. TOOL_CALL/TOOL_RESULT 는 `concurrent` 일 때만 attribution 전달(단일 턴 생략=회귀 0). `ToolCallBubble`/`ToolResultBubble` — props `attribution?` 추가, 있을 때만 `↳@<질문자>` 마이크로라벨(term-faint). `Thread.tsx` — `resolveActiveTurnsCount`·`buildAttribution`·`renderAttrMap`(동시 시 TOOL_* 상속 머지) useMemo, `scrollToMessage`(CSS.escape + reduced-motion `behavior:'auto'`), 비-sticky 세션 행에 `◉ {count}개 작업 진행 중` 배지(≥1 표시·≥2 term-amber 강조), 버블 리스트 상단 동시 2턴+ 구분 띠(`── 동시 스트리밍 중 (N턴) ──`, term-dim-2), 각 버블 `data-msg-id` 래퍼 + props 주입. `Composer.tsx` — `hasMyActiveTurn` 구독 `myTurnBusy`로 `canSend`/`handleSend` 게이팅 + `thread.myTurnBusy` 안내 행(기존 `agentStreaming` steer row 무변경). `index.css` — `.term-dots` keyframe + `prefers-reduced-motion` 정지. `dicts/thread.ts` — ko/en `you`·`someoneElse`·`myTurnBusy`·`activeTurnsBadge`·`concurrentDivider`·`writing`·`replyToAnchorAria`(`◉`/`──` 글리프는 JSX, 텍스트만 i18n). 단위 테스트는 vitest 도입(`vitest.config.ts` node env·`package.json` `test:"vitest run"`·`tsconfig.json` `exclude:["src/**/*.test.ts"]`로 빌드 비끌림).
- **한계(문서화)**: ① **peer 실명 미로딩**(`MessageCreatedPayload`/`toMessage`는 authorId만 — self/작성자 외 username 없음) → 그 외 참여자는 `someoneElse` 폴백. ② **TOOL 귀속 seq 근사**(tool 이벤트에 turnId 부재 → 직전 AGENT_REPLY 상속, 동시 다수 턴에서 부정확 가능; 백엔드가 turnId 실으면 정확화 — 후속). ③ **고밀도 접기**(동시 다수 턴 접기→1줄 요약, WIREFRAME §6 항목6)는 우선순위 낮음 → **후속 WP**(배지+귀속+구분 띠+게이팅+감속모드만 1차 머지).
- **검증(③) — 실측(GR-1 ③)**: `cd frontend && npx tsc --noEmit` **클린(EXIT 0)**. `cd frontend && npm run build`(`tsc -b && vite build`) **EXIT 0**(101 modules, built in ~2.1s — `*.test.ts` 빌드 비끌림 확인). `cd frontend && npx vitest run` **EXIT 0 — 1파일/19테스트 통과**(countActiveStreamingTurns 2·resolveActiveTurnsCount 서버>0/0·null·undefined 폴백·hasMyActiveTurn 내턴 true/남턴 false/HUMAN 미수신 false/null false/COMPLETE false·buildAttribution 내질문 isMine·youLabel 폴백·작성자·peer someoneElse·replyToId null·non-AGENT_REPLY 미포함·attributeToolBubble 단일 상속/선행 없음 null/미존재 null/동시 2턴 인접). backend **무변경**(`git status` 추적 backend 변경 0) + `cd backend && npx tsc --noEmit` **클린(EXIT 0)**. 하드코딩 사용자 노출 문자열 **0**(텍스트는 전부 `t()` — `◉`/`──`/`↳` 글리프만 JSX), 새 색 도입 **0**(term-* 토큰만). **리뷰 3관점 통과** — ① 디자인 충실도(WIREFRAME §6 정본: 귀속 라벨·동시 칩 위계 승격·헤더 배지·구분 띠·내턴 게이팅·reduced-motion 모두 term-*/i18n로 구현), ② 정확성/회귀(스트림·스토어 누적 경로 무변경, 단일 턴 레거시 UX 픽셀 동치, 순수 셀렉터 19/19 단위 검증), ③ 접근성(reduced-motion 점/스파클 정지·정적 `작성 중` 텍스트, 앵커 button aria) — **차단 결함 0**. **브라우저 시각 E2E 수행·통과**(아래 불릿).
- **라이브 브라우저 E2E 실측(Chrome MCP · 실행 중 dev 5173/3001 · 실 LLM · concurrent=true 게시글에 2 사용자(alice·bob) 동시 턴)**: **동시 스트리밍 중**(activeTurns=2) — 헤더 배지 `◉ 2개 작업 진행 중`, 동시 턴 구분 띠 `동시 스트리밍 중 (2턴)`, 두 AGENT_REPLY 귀속 라벨(`↳ @fe-alice-e2xk2` 본인 + `↳ @다른 참여자` peer=someoneElse 폴백), 본인 입장 Composer **잠김**(내턴 게이팅) 모두 DOM 실측 확인. **완료 후**(idle) — 배지/띠 소멸·본인 귀속만 잔존(단일 턴 규칙)·Composer **해제** 확인 → 전 라이프사이클이 WIREFRAME §6 스펙과 일치. (피크 순간 스크린샷은 동시 스트리밍 렌더 부하로 CDP 캡처 타임아웃 — 기능 결함 아님, DOM 텍스트로 결정적 확인.)
- 변경 파일: `frontend/src/lib/threadSelectors.ts`(신규), `frontend/src/lib/threadSelectors.test.ts`(신규), `frontend/vitest.config.ts`(신규), `frontend/src/stores/threadStore.ts`, `frontend/src/stream/useThreadStream.ts`, `frontend/src/components/ChatBubble.tsx`, `frontend/src/components/ToolCallBubble.tsx`, `frontend/src/components/ToolResultBubble.tsx`, `frontend/src/pages/Thread.tsx`, `frontend/src/components/Composer.tsx`, `frontend/src/index.css`, `frontend/src/i18n/dicts/thread.ts`, `frontend/package.json`, `frontend/tsconfig.json`, `frontend/package-lock.json`, `docs/IMPLEMENTATION_NOTES.md`.
