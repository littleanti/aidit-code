...
|---|---|---|---|
| E1 | 직렬 실행기 Ablation **✅W1 실측 완료(2026-07-28)** | "락 없는 동시 쓰기 → 파일 깨짐; 직렬화로 0" (§IV-B) / "완료순 원자 커밋 → 짝 정합" (§IV-C) | §VI-F (신설) |
...
### ✅ W1 실측 결과 (2026-07-28) — CONFIRMED

> 하네스: `backend/bench/e1-ablation.mjs` · 실행 `npm run bench:e1` · 원자료 `bench/out/e1-ablation.json`
> 측정 층: 에이전트/LLM 스택을 거치지 않고 `executeTool`(FILE_WRITE) + 락 프리미티브를 **직접** 구동
> (설계 방침대로 — I/O 경합을 재는 실험이므로 결정적 백엔드가 옳다).
> 조건: writer 4명 × 각 12회 × 페이로드 4 MiB, 같은 파일 `x.txt` 전체 쓰기, 반복 3회 합산.

| 조건 | 분류 스냅샷 | intact | **mixed(torn)** | broken | partial | **위반률** | 최종 파일 |
|---|---|---|---|---|---|---|---|
| `LOCK=on`(현행) | 158 | 0 | **0** | 0 | 158 | **0.00%** | intact ×3 |
| `LOCK=off`(ablation) | 50 | 0 | **37** | 0 | 13 | **74.00%** | **mixed ×3** |

**판정: CONFIRMED — 락이 있으면 0, 빼면 깨진다.**

- `LOCK=off` 에서는 3회 모두 **최종 파일 자체가 오염**됐다(두 writer 의 마커가 한 파일에 공존).
  즉 중간 상태만 잠깐 깨지는 게 아니라 **결과물이 망가진다**.
- `LOCK=on` 의 158개 스냅샷이 전부 `partial` 인 것은 정상이다 — `writeFile` 은 truncate 후 기록하므로
  "아직 다 안 쓴" 중간 상태가 관측되는 것이며, **두 writer 가 섞인 적은 한 번도 없다**(mixed=0).
  최종 파일은 3회 모두 체크섬까지 일치(`intact`).

**감도에 대한 정직한 기록**: 첫 시도(페이로드 512 KiB · writer 2명)에서는 `LOCK=off` 에서도 위반이
**0** 이었다 — 설계 문서가 미리 경고한 대로 OS 단일 `write()` 원자성에 가려진 것이다. 지침에 따라
페이로드를 **4 MiB**, writer 를 **4명**으로 올려 재측정해 위 결과를 얻었다. 즉 이 결함은
**충분히 큰 쓰기에서만** 드러난다 — 작은 파일만 다루는 워크로드라면 락 없이도 우연히 안전할 수 있다.
그 사실 자체가 "락이 왜 필요한가"의 정확한 답이다: **크기에 의존하지 않는 보장**을 준다.

**미실시**: W1b(디렉토리 레이스), W2(convo `tool_calls`↔`role:tool` 짝 정합 — naive 커밋 대조).
W2 는 워커 커밋 경로에 실험 게이트가 필요해 별도 작업이다.
...
### ⚠️ E2-B (2026-07-28) — 두 턴이 **모두 도구를 쓸 때**: 이점이 거의 사라진다

> 위 결과의 한계로 적어둔 "행복 경로"를 실제로 측정한 것이다. 하네스: `BOTH_TOOLS=1`.
> A는 길이 L의 작업, **B도 2초짜리 도구를 실행**해 샌드박스 락을 다툰다.
> 원자료: `e2-hol.mockboth.jsonl`(결정적), `e2-hol.realpilot.jsonl`(실 LLM).

| 시나리오 | C-FIFO | C-PAR | 개선 |
|---|---|---|---|
| **B가 추론만**(원 E2, L=15s) | 14930ms | **235ms** | **63.5×** |
| B도 도구 사용 · mock(결정적, L=6s) | 8262ms | **7735ms** | **1.07×** |
| B도 도구 사용 · 실 LLM(L=6s) | 14341ms | **10624ms** | **1.35×** |

**결론: 병렬 추론의 이득은 턴에서 "추론이 차지하는 비중"에 비례한다.**
두 동시 턴이 모두 부수효과를 내면 **직렬 실행기가 병목**이 되어 이점이 1.07×까지 붕괴한다.
락이 B의 도구를 A의 도구 뒤로 줄 세우고, 그 대기가 B의 응답 지연 대부분을 차지하기 때문이다.

**세부 관찰**

- 실 LLM(1.35×)이 mock(1.07×)보다 이점이 큰 이유: 실 추론은 호출당 수 초가 걸리고 **그 부분은
  실제로 병렬화**된다. mock은 추론을 200ms로 주입했으므로 병렬화할 몫이 거의 없다.
  즉 **실제 모델을 쓸수록 이 시나리오에서도 이점이 커진다** — 다만 63×가 아니라 1.3× 수준이다.
- `bTtftMs`와 `bCompleteMs`가 20ms 차이로 붕괴했다(실 LLM). gpt-4o-mini가 **서두 텍스트 없이
  곧바로 `tool_call`을 방출**하기 때문이다. 따라서 이 시나리오에서 "첫 토큰"은 추론 시작이 아니라
  **도구 왕복 이후**를 재고 있다. 모델이 도구 호출 전에 한두 문장을 먼저 말하는 방식이라면
  체감 지연은 다시 크게 줄어든다(모델·프롬프트 의존).

**그래서 주장은 이렇게 좁혀야 한다**

> v2의 큰 이점(수십 배)은 **동시 질문이 추론 중심일 때** 성립한다 —
> 설명 요청, 코드 리뷰, 질의응답처럼 샌드박스를 건드리지 않는 작업.
> 동시 질문이 **모두 파일을 수정**하면 직렬 실행기가 병목이 되어 이점은 1.1~1.4×로 줄어든다.
> 이것은 설계 결함이 아니라 계약의 필연적 귀결이다(부수효과를 직렬화하기로 **선택**했으므로).
> 다만 그 대가로 얻는 것은 여전히 유효하다 — 파일 깨짐 0, 머지 불필요, 1:1 귀속.

후속(미실시): 같은 파일 vs 다른 파일 구분(현재 락은 샌드박스 단위 — 파일 단위로 좁히면
서로 다른 파일을 만지는 턴은 병렬화 가능), B의 도구 길이 스윕, 실 LLM n 확대(현재 n=2).

### ❗ XC-SCOPE (2026-07-28) — 락을 파일 단위로 좁혀도 **측정 이득이 0이다**

E2-B의 1.07× 붕괴를 고치려고 부수효과 락을 샌드박스 단위 → 충돌 단위(파일)로 세분화했다.
구현·안전성은 검증됐지만(`sandboxLockScope.test.ts` 13건), **종단 성능 이득은 없었다.**
조작 변인 `LOCK_SCOPE=sandbox|file`, 나머지 조건 동일:

| 시나리오 | `LOCK_SCOPE=sandbox` | `LOCK_SCOPE=file` | 차이 |
|---|---|---|---|
| E2-B: A가 SHELL 6s + B도 도구 | 8206ms | 8212ms | **0%** |
| 서로 **다른 파일**에 각 40회 write | 1001ms | 980ms | 2%(노이즈) |
| **같은 파일**에 각 40회 write(대조) | — | 1009ms | — |

**왜 이득이 없나 — 락은 애초에 병목이 아니었다**

1. **SHELL 은 어느 설정에서도 배타다**(어떤 파일을 만질지 알 수 없으므로). E2-B의 붕괴는
   A의 SHELL 도구가 락을 **6초간 보유**한 탓이고, 이건 입도 조정으로 고칠 수 없다.
   두 설정의 차이가 6ms(0%)라는 것이 이 귀속을 확정한다.
2. **파일 쓰기 경합은 원래 미미했다.** `write_file` 1회의 락 보유 시간은 ms 급인데,
   쓰기 1회당 비용은 워커↔부모 ack 왕복 + **새 LLM completion 호출**이 지배한다.
   락을 잡고 있는 시간이 사이클의 극히 일부라 서로 다른 턴의 쓰기가 실제로 겹칠 확률이 낮다.
   같은 파일(1009ms)과 다른 파일(980ms)이 사실상 같다는 것이 그 증거다 —
   **직렬화 여부가 결과를 바꾸지 못하는 수준의 경합**이었다.

**그래서 이 변경을 유지하는 이유(성능이 아니라 보험)**

- 안전 보장은 동일하고 입도만 세분화됐다(호환성 행렬 13건 전수 검증, `LOCK_SCOPE=sandbox` 즉시 롤백).
- **컨테이너 격리를 적용하면 판단이 뒤집힌다**: 부록 B의 계획대로 도구 호출마다 컨테이너를 띄우면
  호출당 200~500ms가 붙어 **락 보유 시간이 지배적**이 된다. 그때는 파일 단위 입도가 실질 이득을 낸다.
  즉 지금은 이득 0이지만, 격리 강화의 **선행 조건**이다.
- 반대로, **E2-B의 1.07×를 개선하려면 락 입도가 아닌 다른 레버가 필요하다**:
  SHELL의 부수효과 범위를 선언하게 하거나(에이전트가 만질 파일을 미리 신고), 샌드박스를
  턴별로 분기해 쓰기-쓰기 충돌만 병합하는 방향. 둘 다 이번 범위 밖이며 설계 재검토가 필요하다.

> 정직한 요약: **요청받은 최적화는 올바르게 구현됐고 안전하지만, 오늘의 워크로드에서는 이득이 없다.**
> "고쳤다"가 아니라 "고칠 수 없는 곳이 어디인지 확정했다"가 이 실험의 성과다.
