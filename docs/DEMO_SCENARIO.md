...
| 8 | B | **ON**·중간 | (9와 **거의 동시에 전송** — *같은 파일* 동시 수정) `vowels.py의 count_vowels가 대문자 모음(A,E,I,O,U)도 세도록 고쳐줘. 먼저 read_file로 vowels.py 현재 내용을 읽고, 파일에 이미 있는 다른 함수는 절대 지우지 말고 그대로 유지한 채 고쳐줘. test_vowels.py에 대문자 케이스도 추가하고 pytest 통과 확인.` |
| 9 | C | **ON**·중간 | (8과 거의 동시에 전송 — **동일 파일 `vowels.py`**) `같은 vowels.py 파일에 자음 개수를 세는 count_consonants(s)를 추가해줘. 먼저 read_file로 현재 내용을 읽고, 기존 count_vowels 함수는 그대로 둔 채 아래에 추가만 해줘. 테스트는 test_consonants.py에 따로 만들고 pytest 통과 확인.` |
| — | 전원 | — | **연출 포인트(v2의 나머지 절반)**: 6·7이 *병렬 추론*을 보여줬다면 8·9는 **직렬 부수효과**를 보여준다. 두 턴의 토큰은 겹쳐서 흐르지만, 툴콜 버블(`▌$ [셸]`·`write_file vowels.py`)은 **절대 겹치지 않고 한 번에 하나씩** 찍힌다 — `withSandboxLock`(샌드박스 단위 mutex)이 파일 쓰기를 직렬화하기 때문. 파일 탭의 `vowels.py` 변경 표시가 두 번 연달아 갱신되는 것도 같이 잡을 것 |
| 10 | A | **ON**·중간 | **정합성 확인(안전망)**: `vowels.py 열어서 count_vowels랑 count_consonants 두 함수가 모두 남아있는지 확인해줘. 하나라도 없어졌으면 복구하고, pytest 전체 돌려서 전부 통과시켜줘.` → 두 사람의 동시 수정이 한 파일에 온전히 합쳐졌음을 화면으로 확인 |
| 11 | C | **ON**·중간 | `리뷰에서 지적한 빈 문자열·대소문자 엣지케이스부터 test에 추가하고, 통과하도록 고쳐줘. 테스트 문자열은 영문 기준으로만.` |
| 12 | A | — | **중단/방향조정 시연**: 11의 에이전트 턴이 도는 동안 A 창 Composer 위의 steer 입력란(`방향 조정 (선택)…`)에 `'12345'나 '#$%'처럼 숫자·특수문자만 있는 문자열 케이스도 넣어줘` 입력 → `■ 중단` 클릭. 에이전트가 방향을 반영해 재개. **steer 이후에는 화면 텍스트 안정화만 보지 말고 그 사용자 Composer의 `내 답변 진행 중` 안내(role=status)가 사라질 때까지 기다릴 것**(빈 입력창에서는 `보내기`가 항상 비활성이라 버튼 상태로는 판정 불가) — 개입된 턴은 텍스트가 멈춘 뒤에도 툴콜을 1분 가까이 더 돌린다(그 사이 같은 사용자로 다음 발화를 보내면 턴이 안 붙고 `세션을 시작하세요`가 뜬다) |
| 13 | C | **ON**·**높음** | `마지막으로 pytest 전체 다 돌려서 결과 보여줘. 실패하는 테스트가 있으면 구현과 테스트를 일관되게 고쳐서 전부 통과시켜줘(기대값만 바꾸지 말고). 그리고 최종 함수 목록·테스트 구조를 정리해줘.` → 터미널 버블 `✓ 실행 완료. [exit 0]` + `===== N passed =====` (초록)로 마무리 |
| 14 | A | **ON**·중간 | **문서 페이오프**: `지금까지 만든 코드를 정리해서 README.md 파일로 작성해줘 — 함수 목록·사용 예시·pytest 실행법 포함.` → 에이전트가 `README.md`를 실제로 써서 파일 트리에 남김(논의→산출물 응결) |

> **8·9(같은 파일 동시 수정)에 대한 정직한 한계** — 서버는 도구 실행을 `withSandboxLock(sandboxId, …)`
> (`backend/src/agent/turn.ts:190`)로 감싸므로 **동시 파일 쓰기 진입은 0**이다(찢어진 파일·인터리브 없음).
> 다만 `write_file`은 **파일 전체 덮어쓰기**라, 두 에이전트가 각자 `read_file` → (상대가 쓰기) → `write_file`
> 순서로 진행하면 **논리적 lost update**(뒤에 쓴 쪽이 앞의 수정을 덮음)가 발생할 수 있다. lock은 이걸
> 막아주지 않는다. 그래서 두 발화 모두 **"먼저 read_file로 읽고 기존 함수는 지우지 말 것"** 을 명시하고,
> 10번 정합성 턴을 안전망으로 둔다. 리허설에서 유실이 반복되면 8·9 전송 간격을 늘리거나(스크립트
> `SAME_FILE_STAGGER_MS`) 8·9 중 하나를 append 성격 지시로 바꾼다.

**엔딩 샷**: 13번의 `===== N passed =====` 초록 터미널 버블 → 14번에서 생성된 `README.md`를 포함해
...
- **같은 파일 동시 수정**(8~10): 두 사람이 **동시에 `vowels.py` 하나**를 고친다. 추론은 겹쳐서 흐르고
  파일 쓰기는 `withSandboxLock`으로 한 번에 하나씩 — **병렬 추론 + 직렬 부수효과**(v2 명제)를 한 화면에서.
  10번 정합성 턴으로 두 수정이 한 파일에 온전히 남았음을 확인.
- **리뷰 루프**(7·11): 에이전트에게 코드 리뷰를 시키고, 지적사항을 골라 수정 지시 — "리뷰".
- **중단/방향조정**(12): 도는 턴에 개입해 방향을 트는 협업 제어.
...
- 6·7단계, 8·9단계 "거의 동시 전송": 오케스트레이터가 B·C 창의 `Promise.all([...])`로 send를 동시 실행.
  8·9는 같은 파일을 건드리므로 스크립트 상단 `SAME_FILE_STAGGER_MS`(기본 0ms)로 간격을 줄 수 있다 —
  0이면 완전 동시, 값을 주면 뒤 발화가 그만큼 늦게 전송돼 lost update 확률이 내려간다(연출은 여전히 동시로 보임).
- 12단계 steer UI는 **턴이 스트리밍 중일 때만** 나타난다. 11단계 전송 직후 A 창에서
...
6. 타임라인 1~14 순차 실행 — 6·7과 8·9는 동시 전송(8·9는 **같은 파일**), 12는 스트리밍 중 개입 (Phase 3).
