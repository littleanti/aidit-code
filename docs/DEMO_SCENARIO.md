...
- 제목: `FastAPI로 TODO API 같이 만들어요`
- 본문(작업 지시): `FastAPI로 TODO CRUD API를 만들어줘. 우선 GET /health 헬스체크 라우트와 pytest 테스트부터 시작하자.`
...
`▌$ [셸] ...` 툴콜 버블과 터미널 결과 버블(`✓ 실행 완료. [exit 0]`)이 이어진다.
...
| # | 창 | AI | 발화 내용 / 행동 |
|---|----|----|------------------|
| 1 | B | **ON**·중간 | `좋네요! POST /todos 생성 API도 추가해줘. 제목(title)만 받으면 되고, in-memory 리스트로 저장해줘.` → 에이전트가 파일 수정 + 테스트 실행 |
| 2 | C | OFF | `잠깐, 스키마부터 정하고 가죠. id는 정수 자동증가, title 필수, done 기본값 false 어때요?` (사람끼리 상의 — AI OFF 시연) |
| 3 | B | OFF | `동의! done 필드까지 포함해서 가요.` |
| 4 | B | **ON**·중간 | `위 논의대로 Todo 모델에 id(자동증가)·title(필수)·done(기본 false)을 반영하고, GET /todos 목록 조회도 추가해줘.` |
| 5 | A | — | 에이전트 턴이 스트리밍되는 동안 **`파일` 탭 클릭** → 파일 트리에 새 파일·변경 표시(호박색 점)가 실시간으로 뜨는 것을 보여줌. 다시 `대화` 탭 복귀 |
| 6 | B | **ON**·중간 | (7과 **거의 동시에 전송** — v2 병렬 협업 시연) `PATCH /todos/{id}로 done 토글하는 API 추가해줘.` |
| 7 | C | **ON**·중간 | (6과 거의 동시에 전송) `지금까지 작성된 코드 리뷰해줘. 에러 처리 빠진 곳이나 상태코드 잘못 쓴 곳 있는지 지적해줘.` |
| — | 전원 | — | **연출 포인트**: `◉ 2개 작업 진행 중` 배지, `동시 스트리밍 중 (2턴)` 구분선, 각 답변의 `↳ @바다` / `↳ @찬` 귀속 칩. 서로 안 기다리고 답이 병렬로 오는 장면 |
| 8 | C | **ON**·중간 | `리뷰에서 지적한 것 중 "없는 id에 404 반환" 먼저 고쳐줘.` |
| 9 | A | — | **중단/방향조정 시연**: 8의 에이전트 턴이 도는 동안 A 창 Composer 위의 steer 입력란(`방향 조정 (선택)…`)에 `테스트도 같이 업데이트해줘` 입력 → `■ 중단` 클릭. 에이전트가 방향을 반영해 재개 |
| 10 | C | **ON**·**높음** | `마지막으로 전체 테스트 다 돌려서 결과 보여주고, 최종 코드 구조를 한 번 정리해줘.` → 터미널 버블 `✓ 실행 완료. [exit 0]` (초록)로 마무리 |
| 11 | A | **ON**·중간 | **문서 페이오프**: `지금까지 만든 코드를 정리해서 README.md 파일로 작성해줘 — 프로젝트 개요·실행 방법·API 엔드포인트 목록 포함.` → 에이전트가 `README.md`를 실제로 써서 파일 트리에 남김(논의→산출물 응결) |

**엔딩 샷**: 11번에서 생성된 `README.md`를 포함해 A 창에서 `파일` 탭으로 전환해 완성된 프로젝트 트리를
천천히 보여주고, 스레드를 스크롤하며 마무리.
