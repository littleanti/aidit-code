...
> **Status: PoC · Version: 0.2 · v2(동시 병렬 협업 에이전트) · Date: 2026-06-29**
...
║         (v0.1 단일 활성 턴 + FIFO 큐 → 화면이 바뀌는 지점만 갱신)        ║
╠══════════════════════════════════════════════════════════════════════╣
║ • 협업 모델은 그대로: 게시글당 단일 세션 · 단일 공유 컨텍스트 · 단일      ║
║   샌드박스. 화면 구조(§6 Thread, §2 Home, §9 Profile)는 유지.           ║
║ • Thread에서 **여러 AGENT_REPLY 버블이 동시 스트리밍** 가능 →            ║
║   다중 "작성 중…" 인디케이터가 공존한다(v0.1은 항상 1개만).              ║
║ • 각 답글이 어느 HUMAN 질문에 대한 답인지 **1:1 시각 연결**(replyToId)    ║
║   — 내 질문↔내 답글 하이라이트/앵커. 배칭 안 함(1턴=1질문=1답).            ║
║ • Thread 헤더 상태 배지: 단일 `● RUNNING` → **`◉ N개 작업 진행 중`**     ║
║   (활성 턴 카운트). 활성 턴 0이면 `○ IDLE`.                              ║
║ • Composer 전송 게이팅을 '세션 단일 streaming' → **'내 턴' 기준**으로     ║
║   해제: 남의 활성 턴 중에도 내 입력은 즉시 전송 가능(HOL blocking 제거). ║
║ • 파일트리: 부수효과가 직렬이라 `file.changed`는 여전히 순차 도착(표시    ║
║   OK), 다만 빠른 연속 갱신을 가정.                                       ║
║ • 색·토큰·레이아웃·반응형 불변. 새 색 없음 — 활성 턴 카운트 배지도        ║
║   기존 term-amber/term-dim만 사용(§12.4 ④에 통합).                       ║
╚══════════════════════════════════════════════════════════════════════╝
```
...
> **(v2 예외)** 단, **동시 병렬 협업 opt-in 체크박스** 1개는 신설한다(아래 폼·불릿). 이는 부모식 "AI 답변 토글"이 아니라 이 글 샌드박스의 **실행 모드**(직렬 vs 병렬)를 생성 시 1회 결정하는 컨트롤이다.
...
│    붙습니다.                       │
│                                  │
│  [ ] 실시간 동시 협업 (실험적)      │  ← (v2) opt-in 체크박스 · 기본 해제
│   ↳ 여러 사람이 동시에 물어도 서로   │     설명(term-dim)
│     기다리지 않고 각자 답이 바로.    │
│   ⚠ 끄면 순차 처리. 켜면 동시       │     경고(term-dim)
│     요청만큼 비용↑ · 같은 파일       │
│     동시수정 시 last-wins.          │
│                                  │
│        [   게시하기   ]            │
...
- **(v2) 동시 병렬 협업 토글**: 본문 아래 **체크박스**(`ConcurrentToggle`, **기본 해제**). 체크하면 이 글 샌드박스가 v2 **동시 병렬 협업** 모드로 생성된다(여러 사용자 질문이 동시 병렬 처리 + 각자 답 즉시 스트리밍, 파일/도구 부수효과는 샌드박스 단위 직렬 안전 적용). 미체크면 v0.1 직렬 동작. **라벨/설명/경고는 i18n**: `t('createPost.concurrent.label')`(**표시 라벨은 사용자 언어로** — 예 "실시간 동시 협업 (실험적)"; '병렬'은 시스템 용어라 화면엔 노출하지 않음, 내부 기능명은 '동시 병렬 협업') / `.desc`(짧은 기능 설명) / `.warning`(⚠ 끄면 순차 처리 · 동시 요청 수만큼 LLM 비용↑ · 같은 파일 동시 수정 시 last-wins) — **하드코딩 금지**. 스타일: 체크 활성 `text-term-amber`, 설명·경고 `text-term-dim`(**새 색 없음**, §12). 게시 시 `POST /posts`에 `concurrent: boolean` 전송 → `Sandbox.meta.concurrentTurns`. **모드는 생성 시 1회 확정**(Thread 진입 후 변경 불가).
...
> 한 번 생성되어 전원에게 동일하게 중계된다(**v2**: 단, 서로 다른 사용자의 턴이 동시에 병렬로 흐를 수 있다 — 각 턴 출력은 1회 생성·전원 중계, §Δv2/§6.2). 사람/에이전트 텍스트 버블에 더해 **도구 호출 버블**·**터미널 출력 버블**·
> (선택)**워크스페이스/파일 트리 패널**·**에이전트/샌드박스 상태 인디케이터**가 추가된다.
>
> **Δ v2 — 여러 AGENT_REPLY 동시 스트리밍.** v0.1은 게시글당 단일 활성 턴 + FIFO 큐라 화면에 "작성 중…" 인디케이터가
> 항상 1개뿐이었다(내 질문이 남의 긴 턴 뒤로 pending). v2는 **추론(LLM 스트리밍)을 병렬화**해 N명의 질문이 각자 독립
> 턴으로 동시에 inflight → **여러 AGENT_REPLY 버블이 동시에 스트리밍**될 수 있다(다중 "작성 중…" 인디케이터 공존).
> 단일 세션·공유 컨텍스트·단일 샌드박스는 그대로이며, **부수효과(파일 쓰기/도구 실행)는 샌드박스 단위로 직렬화**되어 파일
> 충돌이 없다. 화면이 바뀌는 지점은 셋: ① **각 답글↔질문 1:1 시각 연결**(replyToId 하이라이트/앵커), ② **헤더 상태 배지가
> "N개 작업 진행 중"(활성 턴 카운트)**, ③ **Composer 전송 게이팅을 '내 턴' 기준으로 해제**(남의 활성 턴 중에도 즉시 전송).

```
┌────────────────────────────┐
│ ‹  FastAPI 헬스체크 만들어줘  🔖 ⋯│  ← 글 상세 헤더(뒤로·제목 중앙·북마크·메뉴)
│  [ ◉ 2개 작업 진행 중 · pi · 3명 ]│  ← v2: 활성 턴 카운트 배지(RUNNING=term-amber). 0이면 ○ IDLE
│ ┌──────────────────────────┐ │
...
> **Δ v2 — 여러 AGENT_REPLY 동시 스트리밍 + 1:1 질문↔답글 연결.** 두 사람이 거의 동시에 AI on으로 질문하면 두 개의
> 에이전트 턴이 병렬로 inflight → 두 AGENT_REPLY 버블이 동시에 스트리밍된다. 각 답글은 **자기가 답하는 HUMAN 질문에 1:1로
> 시각 연결**(replyToId)되며, **내 질문↔내 답글은 하이라이트/앵커**(`↳ @내질문`)로 강조해 N개가 섞여도 흐름을 잃지 않는다.
> ```
> ┌────────────────────────────┐
> │ ‹ FastAPI 헬스체크 만들어줘  🔖 ⋯│
> │  [ ◉ 2개 작업 진행 중 · pi · 3명 ]│  ← 활성 턴 2개(term-amber)
> │ ──────── 세션 ────────        │
> │        나 ▶ pytest도 붙여줘 ›┐(👩)│  ← 내 질문(우, CTA) — 앵커 대상
> │              방금 ✓     │
> │ (🟢)minji                  │
> │  └‹ 그럼 Dockerfile 도?       │  ← minji 질문(좌)
> │ ── 동시 스트리밍 중 (2턴) ──   │  ← 동시 턴 구분 띠(term-dim-2; 2턴+ 시 표시·load-bearing)
> │ (⚡)pi agent [AGENT]  ↳@나     │  ← 내 질문 답글(앵커 하이라이트=term-active 좌보더)
> │  └‹ test_health.py 를 추가… ▌ │     스트리밍 ▌ 커서
> │ (⚡)pi agent [AGENT] ↳@minji  │  ← minji 질문 답글(다른 턴, 동시 스트리밍)
> │  └‹ ✦ 작성 중… •••           │     아직 PENDING → 스파클 인디케이터
> │ ──────────────────────────  │
> │ (＋)( 메시지를 입력하세요…)[🤖⌄](↑)│  ← 내 입력은 위 2턴과 무관하게 즉시 전송 가능
> └────────────────────────────┘
> ```
> - **앵커/하이라이트**: 각 AGENT_REPLY 버블 상단에 `↳ @<질문자>` 마이크로라벨(`term-dim`). **본인 질문에 대한 답글**은
>   좌측 보더를 `term-active`로 강조하고, 탭하면 대응 HUMAN 버블로 스크롤 앵커(반대 방향도). 새 색 없음 — 강조는 기존 `term-active`.
> - **다중 인디케이터 공존**: 동시에 스트리밍 중인 각 턴은 자기 `▌` 커서(누적 본문 끝) 또는 `✦ 작성 중… •••`(PENDING)를
>   **독립적으로** 표시한다. v0.1처럼 화면 전체에 하나만 뜨지 않는다.
> - **Composer 게이팅**: 전송 가능 여부는 **'내 활성 턴' 기준**(내가 보낸 마지막 턴이 진행 중일 때만 내 전송 잠금/안내).
>   남이 만든 활성 턴은 내 전송을 막지 않는다(HOL blocking 제거). 단, 샌드박스 동시 턴 상한(§13) 초과 시에는 공정 큐 대기 안내.
> - **1인 1활성턴(self-concurrency = 1)**: 한 사용자는 동시에 **자기 턴 1개만** 진행한다. 내 턴이 inflight인 동안 **내 Composer는 잠기고**(전송 시 `내 답변 진행 중 — 끝나면 보낼 수 있어요` 안내, i18n `thread.myTurnBusy`), 남의 턴은 내 전송을 막지 않는다. 병렬은 **사용자 간**에만 발생 → 동시 스트리밍 버블 수 ≤ 동시 활성 사용자 수(자연 상한).
> - **고밀도(동시 턴 다수) 처리**: 동시 스트리밍 턴이 많아지면 **내 턴은 펼치고 남의 진행 턴은 접어** 1줄 요약(`(⚡)@<질문자> ✦ 작성 중… ›`, 탭하면 펼침)으로 표시한다. 헤더 배지 `◉ N개 작업 진행 중`이 전체 수를 알린다. 새 색 없음(term-amber/term-dim).
...
| **AGENT_REPLY** | 좌측 | 좌측(⚡ pi 마크) | 앰버 틴트(`rgba(60,48,10,0.22)` 보더 `term-amber-line`) + `pi agent` + `[AGENT]` 라벨(`term-amber`). **v2: 상단 `↳ @<질문자>` 1:1 라벨**(기본 `term-dim`, **동시 2턴+ 시 `term-amber-line` 박스 칩으로 승격** — "누구 답인가"는 동시 모드의 핵심 정보라 위계 강조); 본인 질문 답글은 좌보더 `term-active` 강조 + 앵커. **여러 버블 동시 스트리밍 가능** |
| **TOOL_CALL** | 좌측(전폭) | 없음 | **`$ <cmd>` 프롬프트 풍**, `term-dim`/`term-faint`, 좌측 `▌` 프롬프트 마크. 도구 이름·인자 요약. **v2(동시): 소속 턴 `↳@<질문자>` 마이크로라벨**(어느 턴의 도구 호출인지 — seq 인터리브 구분; 단일 턴이면 생략) |
| **TOOL_RESULT** | 좌측(전폭) | 없음 | **고정폭 출력 + 스크롤 컨테이너**(`overflow-x:auto`). 성공=기본색·`[exit 0]`/`✓`, 실패=`term-red`·`[exit N]`. **v2(동시): 짝 TOOL_CALL과 같은 `↳@<질문자>` 턴 귀속**(인터리브 구분) |
| **SYSTEM** | 중앙(전폭 띠) | 없음 | `term-dim-3` 미세 텍스트(세션 상태/오류 안내). 예: `— 세션 일시중단됨 (SUSPENDED) —` |
| 로딩(사람) | 해당 위치 | 해당 | `⟳ 입력 중…` 점 애니메이션 |
| 스트리밍(에이전트) | 좌측 | ⚡ | **`✦ 작성 중… •••`**(스파클 + 점) 또는 누적 본문 끝 `▌` 커서. **v2: 동시 스트리밍 중인 각 턴이 자기 인디케이터를 독립 표시**(화면 전역 단일 아님). **`prefers-reduced-motion`: 점/커서 애니메이션 정지 → 정적 `작성 중` 텍스트**(동시 N개일수록 모션 총량↑ — 감속 모드 준수 필수) |
...
> **Δ v2 — 병렬 추론 + 직렬 부수효과(타임라인 정정).** 위 단일 턴 흐름은 한 턴 내부에서는 그대로 유효하나, v2에서는
> **여러 턴이 동시에 inflight**할 수 있다. 화면/이벤트 관점의 정정:
> - **1턴 = 1 HUMAN 질문 = 1 AGENT_REPLY**(replyToId 1:1). `message.created(AGENT_REPLY/PENDING)`는 자신이 답하는
>   HUMAN 메시지의 `replyToId`를 포함하며, 화면은 이를 `↳ @<질문자>` 라벨/앵커로 연결한다(배칭 없음).
> - **t1~t7이 턴마다 병렬로 발생**: `agent.token`·`tool.call`·`tool.output`·`message.updated`는 모두 `turnId`(또는
>   `messageId`)로 라우팅되어 **각 AGENT_REPLY 버블에 독립적으로** 누적된다. 두 버블이 동시에 토큰을 받아 함께 타이핑된다.
> - **세션 상태는 단일 RUNNING/IDLE이 아니라 활성 턴 카운트**: `session.status=RUNNING`은 v0.1에서 "이 세션에 진행 중
>   턴이 있음"을 뜻했고, v2에서는 헤더 배지가 **활성 턴 수 N**(`◉ N개 작업 진행 중`)을 표시한다. 한 턴이 COMPLETE되어도
>   다른 턴이 남아 있으면 배지는 RUNNING을 유지하며, **활성 턴 0일 때만 `○ IDLE`**.
> - **부수효과 직렬화(t3~t6)**: 도구 실행·파일 쓰기·컨텍스트 커밋은 **샌드박스 단위 단일 직렬 실행기**를 통과한다. 따라서
>   `tool.call`/`file.changed`는 (병렬 추론과 달리) **여전히 순차 도착**하며 파일 충돌이 없다(워크스페이스 패널 표시 로직 불변).
> - **인터럽트/스티어링은 턴 단위**: `POST .../interrupt`는 `turnId`를 받아 **해당 턴만** INTERRUPTED로 만든다(다른 사람의
>   동시 턴은 영향 없음). turnId 미지정 호환 시에는 내 가장 최근 활성 턴을 대상으로 한다.
> - **동시성 상한**: 샌드박스당 동시 inflight 턴 수 cap(초과분은 공정 큐). 화면은 큐 대기 시 "잠시 후 시작…" SYSTEM/안내.
...
턴 큐 대기(v2): 동시 턴 상한 초과 시 내 AGENT_REPLY placeholder에 "잠시 후 시작…"(term-dim) → 슬롯 나면 스트리밍
```

> **Δ v2 — 동시 턴 상태.** 여러 AGENT_REPLY가 동시에 스트리밍 중일 때 한 턴이 실패/인터럽트되어도 다른 턴은 영향받지
> 않는다(해당 버블만 term-red/부분 COMPLETE 확정). 헤더 배지의 활성 턴 카운트는 각 턴 종료마다 감소하며, **마지막 턴이
> 끝나야 `○ IDLE`**로 돌아간다. 동시 토큰은 본질적으로 턴 수만큼 비용이 든다(알려진 한계 — TRD/PRD 참조).
...
| `ConcurrentToggle` *(v2)* | CreatePost(동시 병렬 협업 opt-in 체크박스 + 설명/경고, i18n, term-amber/term-dim) | FR-3.6 |
| `Avatar`(user/me/agent) | Thread/원본카드 | FR-5 |
| `ChatBubble`(human/agent/tool-call/tool-result/system) | Thread | FR-5 |
| `Composer`(AI on/off 토글, **v2: '내 턴' 기준 게이팅**) | Thread | FR-5.1 |
| `ToolCallBubble` / `ToolResultBubble` | Thread | FR-6 (신규) |
| `WorkspacePanel`(파일 트리/내용) | Thread | FR-6 (신규) |
| `SandboxStatusBadge` / `SessionStatusBadge`(**v2: 활성 턴 카운트**) | Home/Thread | FR-5 (신규, Gemini 배지 대체) |
...
| **④ 에이전트/샌드박스 상태 배지**(`SandboxStatusBadge`/`SessionStatusBadge`) | **GEMINI 배지의 대체물.** LED 점 + 대문자 라벨. **활성(RUNNING) = `term-amber`**(점 글로우), **READY/IDLE/SUSPENDED(유휴) = `term-dim`**, **ERROR/FAILED = `term-red`**(`animate-pulse`), **CREATING/STARTING = `term-dim` + `⟳`**. Home 카드(샌드박스 상태 요약)에 렌더. **Δ v2 — Thread 헤더는 단일 RUNNING이 아니라 활성 턴 카운트를 표시**: 활성 턴 ≥1이면 `◉ N개 작업 진행 중 · pi · M명`(점 `term-amber`), 활성 턴 0이면 `○ IDLE`(`term-dim`). N(활성 턴 수)·M(attach 인원)은 고정폭 `tabular`. **새 색 없음**(term-amber/term-dim만). hover 시 KO/EN 툴팁(`thread.activeTurns` 등). |
...
- **Δ v2 — 병렬 추론 / 직렬 부수효과 경계**: 추론(스트리밍)은 턴별 병렬이지만 **파일 쓰기·도구 실행은 직렬화**되어
  동시 파일 쓰기 진입이 0이다(파일 충돌·머지 불필요, last-wins 적용). 샌드박스당 동시 inflight 턴 수에 상한을 두고 초과분은 공정 큐로
  대기시켜 비용/부하를 제어한다(화면은 "잠시 후 시작…"으로 안내). 단일 공유 컨텍스트는 유지된다(per-user 프로세스 격리 아님).
  - **(2026-07-28 XC-SCOPE 갱신)** 직렬 경계는 샌드박스 단위 → **충돌 단위**로 좁혀졌다(TRD §6.4):
    서로 **다른 파일**을 쓰는 턴은 병렬 통과, 같은 파일·쉘·삭제는 배타. **화면 동작·경고 문구는 바뀌지 않는다**
    — 사용자에게 노출되는 `last-wins` 안내(§CreatePost 토글 경고)는 여전히 정확하다(락은 쓰기 원자성만
    보장하고 read→write 논리 레이스는 남는다). 본 문서의 다른 "샌드박스 단위 직렬" 서술도 이 정의로 읽는다.
